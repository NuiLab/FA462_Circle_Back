...
using TMPro;
using UnityEngine;
using UnityEngine.SceneManagement; // Essential for scene loading
    ...
    public class AnimateText : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyDown(KeyCode.Space) && _cycleCoroutine == null)
                _cycleCoroutine = StartCoroutine(CycleThroughLinesAndEndScene()); 
        }

        private IEnumerator CycleThroughLinesAndEndScene() 
        {
       
            yield return StartCoroutine(CycleThroughLines());
        ...
        /// </summary>
        private IEnumerator CycleThroughLines() // <<--- PULL ORIGINAL LOGIC INTO A NEW METHOD
        {
            ...
            {
                // ... (Original logic for setting text and fading in/out) ...
